...
def getResponse(transcript, topic, category='transcript',
                end_feature='assigment or homework'):
    ...
    if cat:
        ...
        template = f'''
            [INST] <<SYS>> 
            {Template.audio_prompts.get(cat).format(topic=Template.topic)} 
            Using this transcript : <</SYS>>''' + '{transcript}[/INST]'

    else:
        template = f'''
            [INST] <<SYS>>
            Write a summary for a {category} on {topic}.
...
def save_summary(filename, topic='some topic', category='lecture', end_feature='homework or assignment'):
    ...
    with open(f'Summaries/{filename}.txt', 'a', encoding='utf-8') as f:
...
def summarize_text(text, topic='The context of transcript', category='transcript', filename=''):
    
    log.debug('Reading file.')
    RESPONSE = getResponse(text, topic=topic, category=category)['text']

    try:
        log.info('Clearing temporay files.')
        delete_temprory_files(filename, 'Transcripts', 'txt')
        delete_temprory_files(filename, 'Recordings', 'mp3')
