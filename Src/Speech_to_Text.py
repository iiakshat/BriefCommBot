...
def generate_transcript(filepath):
    ...
    result = model.transcribe('Src/Recordings/'+filepath, verbose=True, task='translate')

    detected_lang = LANGUAGES[result['language']].capitalize()
    print(detected_lang)
    transcript = result['text']

    output_path = f"Src/Transcripts/{filename}.txt"
    print(transcript)
    try:
        os.remove('Src/Recordings'+filepath)
    except:
